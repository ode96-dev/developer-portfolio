...
const Navbar = () => {
  ...
  const handleNavClick = (sectionsId: string) => {
    scrollToSection(sectionsId);
    setIsMenuOpen(false);
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="backdrop-blur-lg border-t px-5 py-6 space-y-3">
          {NAV_LINKS.map((link) => (
            <button
              key={link.id}
              onClick={() => handleNavClick(link.id)}
              className={`block w-full text-left px-4 py-3 rounded-lg font-medium transition-all duration-300 ${activeSection === link.id ? "text-white bg-white/50" : "text-white/70 hover:text-white hover:bg-white/5"}`}
            >
              {link.label}
            </button>
          ))}
